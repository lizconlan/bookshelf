...
At some point, I'm going to need a Librarian app [as of 2026, we now have one of those - Ed.] to manage the metadata and images. Manually creating JSON files and creating screenshot images is pretty annoying at scale (it turns out).
...
The easiest way to add a new book is - having clicked the `+ Add Book` button - to paste the ISBN 10 or 13 into the ISBN field and it should fetch the rest from openlibrary.org (although you might want to check the publisher names to make sure they fit the filters you're expecting in the dropdown - Inc. vs Incorporated. vs a different publisher taking over).
